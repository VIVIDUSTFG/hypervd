...
class HypClassifier(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.cls = ManifoldParameter(self.manifold.random_normal(
            (args.num_classes, self.input_dim), std=1./math.sqrt(self.input_dim)), manifold=self.manifold)
        if args.bias:
...
class DistanceAdj(Module):
    ...
    def forward(self, batch_size, max_seqlen, args):
        ...
        self.dist = torch.unsqueeze(self.dist, 0).repeat(
            batch_size, 1, 1).to(args.device)
        return self.dist
